...
- **derived query method 우선**: Spring Data JPA 의 메서드 이름 기반 쿼리(`findBy...And...In...OrderBy...`) 로 표현 가능하면 `@Query` 보다 우선 사용한다. JPQL 본문의 enum 전체 경로 이름/JPQL 문법 잡음을 피하고 타입 안전성을 얻는다. 조건이 4~5개 이상으로 메서드명이 흉해지면 그때 `@Query` 로 전환.
  ```java
  // ✅ derived (단순 조건)
