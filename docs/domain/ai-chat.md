...
**AiChatTurnRequest** — 한 턴의 요청 기록. `requestId` 멱등 판정과 미정산 예약 반환의 정본이다.
...
미정산 예약 반환의 판정 기준이 된다. 자세한 흐름은 아래 "메시지 전송" 시나리오.
...
미정산 예약 반환이 이미 끝낸 요청이면 예산을 건드리지 않고 409(`TURN_REQUEST_ALREADY_FINISHED`)로
...
- **멈추는 조건 셋**(하나만 걸려도 멈춘다): ① 최초 1회 + 다시 시도 2회, 합쳐 3회. 간격은 1초 고정 —
  ...
  **턴의 만료 시각**(진행 목록 등록 시각 + 만료 유예 40초 − 여유 2초)을 넘으면 시작하지 않는다 —
  만료 뒤에는 미정산 예약 반환이 같은 행을 닫을 수 있어 겹쳐 봐야 헛일이다. ③ 다시 시도할 예외가 아니다.
  최악의 소요(3회 × (연결 획득 3초 + 잠금 대기 5초) = 24초)는 후처리 여유 10초를 넘지만, 조건 ②가
  만료 시각에서 먼저 끊는다.
...
| 기한 | 무엇을 재는가 | 시작점 |
|---|---|---|
| 생성 전체 기한 (20초) | 생성 수신 전체에 허용하는 절대 상한 | 외부 API 호출 시작 |
| 무응답 기한 (10초) | 다음 데이터를 기다리는 시간 | 스트림 시작, 그 뒤로는 청크 1건을 받을 때마다 다시 시작 |
| 전달 기한 (30초) | SSE 쓰기를 계속할 수 있는 시간 | 전달 채널 생성(선행 처리 통과 직후, `SseEmitter` 를 돌려주기 직전) |
| 종료 대기 상한 (60초) | 배포 때 진행 중인 턴을 기다리는 시간 | shutdown 신호 |

요청 행의 만료 시각(`expires_at`)은 **접수 시각에서 파생되고 그 뒤로 갱신되지 않는다** — 접수부터
40초가 한 턴의 시간 예산이고, 진행 상황에 따라 그 선을 뒤로 미루면 상한 구실을 못 한다.
갱신하려면 진행 중인 실행이 살아 있다는 신호를 주기적으로 DB 에 써야 하는데, 그 신호를 쓸 수 없게 된
경우(프로세스 사망)가 바로 반환이 고치려는 상황이라 순환이 된다.
...
**미정산 예약 반환** — 선불 예약이 걸린 뒤 그 턴을 끝내는 코드가 영영 돌지 않으면(프로세스 사망,
종료 중 후처리 제출 거절, 후처리 예외) 예약이 그 사용자의 일일 예산에 그날 내내 묶인다.
`AiChatExpiredTurnScheduler` 가 주기적으로 기한이 지난 미종료 요청을 훑어 `EXPIRED` 로 확정하고
예약을 돌려준다. **생성을 다시 실행하지 않고 청구하지도 않는다** — 만료는 "이 요청을 더 기다리지
않고 예약을 돌려준다" 는 정산 결정이다(저장과 청구가 같은 트랜잭션이라 "저장됐는데 미청구" 는
존재하지 않는다). 시간이 지났다는 것이 실행이 사라졌다는 증명은 아니므로, 확정하는 순간 같은 행을
잠그고 미종료·기한 초과를 다시 확인한다. 뒤늦게 성공한 실행은 같은 잠금·확인에 걸려 DB 에
반영되지 않는다 — 정확성을 보장하는 것은 시간 여유가 아니라 이 잠금이라, 만료 판정에 유예를 두지 않는다.
...
| 스트리밍 기한·버퍼 (`ai-chat.streaming`) | **모두 후보값** — 기한 셋은 "실제 생성은 길어야 10~15초" 라는 사용자 판단에 맞춘 값이고, 실측 분포는 아직 없다. 생성 전체 20초 / 무응답 10초 / 전달 30초 / 종료 대기 60초, 연결당 전달 큐 256건. 여유 둘(선행 10초·후처리 10초)은 기한이 아니라 요청 만료 시각 산식에만 쓴다. 이 값들과 구간별 상한(moderation HTTP 6초 · Redis 명령 기한 1초 × 선행 처리의 Redis 호출 2회 · Hikari 연결 획득 3초)이 앞뒤가 맞는지는 기동 시 `AiChatTimeBudgetValidator` 가 대조하고 어긋나면 기동을 막는다 |
| 요청 만료·미정산 예약 반환 (`ai-chat.turn-recovery`) | **모두 후보값.** `expires_at` = 접수 + (선행 여유 10 + 생성 전체 20 + 후처리 여유 10) = **40초** — 한 턴의 시간 예산이다. 유예 0초라 넘긴 행은 곧바로 `EXPIRED` 로 확정한다(정확성은 행 잠금이 보장). 스캔 60초 주기, 한 번에 100행씩 읽되 묶음이 꽉 차면 같은 스캔이 이어 읽어 밀린 양을 다 비운다. 예약 반환은 최악 접수 후 약 1분 40초 |
| 감상문 생성 자격 | 누적 토큰 500 이상 (`SummaryDraftPolicy` 상수, 정책 확정 전 임시값) |

**부하 재측정 (2026-09-06, 로컬 mock 환경).** 이 문서가 설명하는 전달 정책과 성공 기준을 실물로 확인했다 —
동시 N=50·100 각 2라운드에서 성공률 100%(HTTP 200 + SSE `done`/`replace` + DB 3조건: 요청 행 `SUCCEEDED` ·
`assistant_message_id` · 정산 행), 느린 클라이언트에서 전달 큐가 차면 델타를 버리고 `replace` 로 끝나며 그 본문이
DB 에 저장된 본문과 정확히 같았고, 연결 단절·전달 기한 만료에서도 저장·정산은 완주했으며, 같은 `requestId` 를
동시에 두 번 보내면 정확히 한 건만 통과했다. **로컬 mock 결과라 운영 용량의 근거는 아니다.** 상세와 한계는
미추적 디렉터리의 보고서에 있다 —
...
- docs/ops/ai-chat-shutdown-and-recovery.md — 배포 종료 대기와 미정산 예약 반환의 역할 분담,
  설정 항목, 수동 실행 테스트
