...
| 스트리밍 기한·버퍼 (`ai-chat.streaming`) | **모두 후보값** — 기한 셋은 "실제 생성은 길어야 10~15초" 라는 사용자 판단에 맞춘 값이고, 실측 분포는 아직 없다. 생성 전체 20초 / 무응답 10초 / 전달 30초 / 종료 대기 60초, 연결당 전달 큐 256건. 여유 둘(선행 60초·후처리 60초)은 기한이 아니라 요청 만료 시각 산식에만 쓴다 |
| 요청 만료·복구 (`ai-chat.turn-recovery`) | **모두 후보값.** `expires_at` = 접수 + (선행 여유 60 + 생성 전체 20 + 후처리 여유 60) = 140초. 복구는 그 뒤 grace 30초를 더 기다린 행만 `EXPIRED` 로 확정(실효 170초). 스캔 60초 주기, 한 번에 100행. 예약 반환은 최악 접수 후 약 3분 50초 |
