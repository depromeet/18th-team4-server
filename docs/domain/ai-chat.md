...
1. 본문 검증 (빈 값·4,000자 초과 → 400)
2. DB 카운트 기반 전송 한도 확인 — 10초 내 정상 5건 / 1시간 내 거부 20건
   초과 시 429. HTTP 인터셉터의 요청 수 제한(아래 정책 값)과는 **별개의 2차
   방어선**이다.
...
5. USER 메시지 `COMPLETED` 저장 → LLM 스트리밍. 출력 쪽 가드레일(프롬프트
...
6. 스트림 에러·클라이언트 중단 시 부분 응답을 `FAILED` 로 저장한다 — 버리지
...
| 메시지 최대 길이 | 4,000자 |
| 전송 한도 (DB 카운트) | 10초/정상 5건 · 1시간/거부 20건 |
| HTTP 요청 한도 (인터셉터) | userId 기준 분당 20 · 일 200 (POST 만, 2026-06-20 429 사고의 처방) |
...
- `infrastructure/ai/openai/` — 포트 구현체, advisor 체인, rate limiter
