...
| 스트리밍 기한·버퍼 (`ai-chat.streaming`) | **모두 후보값** — 기한 셋은 "실제 생성은 길어야 10~15초" 라는 사용자 판단에 맞춘 값이고, 실측 분포는 아직 없다. 생성 전체 20초 / 무응답 10초 / 전달 30초 / 종료 대기 60초, 연결당 전달 큐 256건. 여유 둘(선행 10초·후처리 10초)은 기한이 아니라 요청 만료 시각 산식에만 쓴다. 이 값들과 구간별 상한(moderation HTTP 8초 · Hikari 연결 획득 3초)이 앞뒤가 맞는지는 기동 시 `AiChatTimeBudgetValidator` 가 대조하고 어긋나면 기동을 막는다 |
