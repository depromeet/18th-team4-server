...
50초가 한 턴의 시간 예산이고, 진행 상황에 따라 그 선을 뒤로 미루면 상한 구실을 못 한다.
...
| 스트리밍 기한·버퍼 (`ai-chat.streaming`) | **모두 후보값** — 기한 셋은 "실제 생성은 길어야 10~15초" 라는 사용자 판단에 맞춘 값이고, 실측 분포는 아직 없다. 생성 전체 20초 / 무응답 10초 / 전달 30초 / 종료 대기 60초, 연결당 전달 큐 256건. 여유 둘(선행 10초·후처리 20초)은 기한이 아니라 요청 만료 시각 산식에만 쓴다. 이 값들과 구간별 상한(moderation HTTP 8초 · Hikari 연결 획득 10초)이 앞뒤가 맞는지는 기동 시 `AiChatTimeBudgetValidator` 가 대조하고 어긋나면 기동을 막는다 |
| 요청 만료·미정산 예약 반환 (`ai-chat.turn-recovery`) | **모두 후보값.** `expires_at` = 접수 + (선행 여유 10 + 생성 전체 20 + 후처리 여유 20) = **50초** — 한 턴의 시간 예산이다. 유예 0초라 넘긴 행은 곧바로 `EXPIRED` 로 확정한다(정확성은 행 잠금이 보장). 스캔 60초 주기, 한 번에 100행씩 읽되 묶음이 꽉 차면 같은 스캔이 이어 읽어 밀린 양을 다 비운다. 예약 반환은 최악 접수 후 약 1분 50초 |
