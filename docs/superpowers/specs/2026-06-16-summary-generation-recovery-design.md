...
> TPM/RPM 은 분당 **속도** 제한이지 "한 건 크기" 제한이 아니다. 큰 대화는 거부되지 않고 분당 예산을 더 먹어 처리량만 깎는다. 한 요청의 **크기 상한은 모델 컨텍스트 윈도우**(`gpt-4o-mini` = 128k 토큰)이며, 이를 넘는 요청만 거부된다(별개 한계). 현재 채팅 대화는 이를 넘을 일이 드묾.
...
### 12.4 RPM/TPM 값을 어떻게 정하나

**천장은 우리가 정하는 게 아니라 OpenAI 가 준다**(모델 + 사용 등급(tier)에 따라). 우리가 설정하는 건 그 천장에서 떼어낸 배치 몫이다.

```
배치 RPM = OpenAI 천장 RPM × 배치 몫 × 안전여유
배치 TPM = OpenAI 천장 TPM × 배치 몫 × 안전여유
```

- **천장 확인**: OpenAI 대시보드(모델별 한도) 또는 **응답 헤더 `x-ratelimit-limit-requests`/`-limit-tokens`**. 후자는 `OpenAiResponseErrorHandler` 가 이미 파싱 중(지금은 429 응답에서만).
- **배치 몫**: 보수적으로 30~40% 부터. ⚠️ 라이브 채팅도 같은 `gpt-4o-mini` 천장을 공유하므로 분리가 실제로 중요.
- **안전여유**: 토큰 추정이 근사라 80~90% 만.
- **처리량 ≈ 배치 TPM / 요약 1건 토큰.** 요약은 입력이 커서 보통 **TPM 이 먼저 빈다**(RPM 보다 결정적).
- **확장성**: 등급(tier)은 누적 사용액으로 자동 상승하며 천장이 10~50배씩 커진다. 우리는 **설정값 두 개만 키우면** 된다. 천장이 낮아 느려도 작업은 큐에 안전히 쌓여 흘러나갈 뿐 — 유실·실패 아님(느림 = 용량 다이얼, 정합성 문제 아님).
- **확장점**: 성공 응답의 `x-ratelimit-remaining-*` 헤더로 양동이를 자동 보정(헤더 파싱 코드 기존 존재 — 429 한정을 성공 응답까지 확장).
