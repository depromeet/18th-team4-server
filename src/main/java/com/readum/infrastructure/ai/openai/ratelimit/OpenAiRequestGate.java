 ...
 * 생성이 실패하면 호출 지점이 확보 때 받은 계상 내역({@link GateReservation})을 {@link #compensate} 에
 * 돌려줘 보상 차감할 수 있다. 실패의 대부분(연결 실패·4xx·429)은 OpenAI 가 토큰을 소모하지 않아
 * 보상이 실제와 맞지만, HTTP 200 을 받은 뒤 응답을 읽거나 변환하다 실패한 경우는 이미 과금된 뒤라
 * 보상이 실제보다 많이 되돌리는 셈이 된다. 그래도 예산이 부풀지는 않는다 — 보상은 확보 당시의 분 키를
 * 되돌리므로, 분을 넘겨 도착한 실패(채팅의 읽기 타임아웃 90초가 대표 사례)는 이미 지나간 창을 건드리고
 * 현재 분의 남은 예산은 그대로다.
