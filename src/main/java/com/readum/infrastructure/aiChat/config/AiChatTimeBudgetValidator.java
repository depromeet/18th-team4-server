...
public class AiChatTimeBudgetValidator {
    ...
    /**
     * 다섯 조건을 차례로 본다. 어긋나면 값과 식을 그대로 담은 {@link IllegalStateException} 을 던져 기동을 막는다.
     *
    ...
    static void verify(
        ...
        if (redisCommandTimeout == null) {
            log.warn("[AI 채팅] Redis 명령 기한을 읽지 못해 선행 처리 여유와 대조하지 못했습니다 — "
                    + "spring.data.redis.timeout 이 설정되어 있지 않습니다(라이브러리 기본 60초). "
                    + "문서(docs/ops/ai-chat-shutdown-and-recovery.md)로 확인하세요.");
        } else {
        ...
        // 5) DB 연결을 빌리는 데만 후처리 여유를 다 쓰면, 저장·정산이 시작도 못 한 채 만료된다.
    ...
    /** Hikari 가 아닌 DataSource(테스트 대역 등)에서는 읽을 값이 없다 — 그때는 {@code null} 을 돌려 조건 5 를 건너뛴다. */
