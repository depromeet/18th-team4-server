...
public interface AiChatSessionRepository extends JpaRepository<AiChatSession, Long> {
     ...
     *  - PENDING, 또는 유효 점유(lockedUntil > now) PROCESSING 작업 존재
     *                                → "SUMMARIZING" (감상문 생성 진행 중 — SummaryJobRepository.existsBlockingSummaryJob 과 동일 조건)
     *  - 그 외                        → "ACTIVE"
     * 점유 만료 PROCESSING 은 "생성 중" 으로 보지 않으므로 lockedUntil > now 로 거른다.
     * PENDING 을 포함하는 이유: 생성 요청은 "요청 시점" 기준이므로 워커가 작업을 집어가기 전에도 화면상 SUMMARIZING 으로 표시해야 한다.
     *
