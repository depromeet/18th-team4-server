 ...
 *
 * <p>미정산 예약 반환은 {@link #findOverdueUnfinishedIds} 로 대상 목록을 훑고, 한 건씩 아래
...
public interface AiChatTurnRequestRepository extends JpaRepository<AiChatTurnRequest, Long> {
    ...
    /**
     * 요청 종료(성공 확정·실패 기록·만료 확정)가 잠글 행을 읽는다 — {@code SELECT ... FOR UPDATE}.
     *
     * <p>잠금이 필요한 이유: 종료는 "미종료인지 확인" 과 "종료 상태로 바꾸고 예산을 되돌린다" 두 단계이고,
     * 그 사이에 다른 실행(늦은 성공 vs 미정산 예약 반환)이 같은 행을 보면 둘 다 미종료로 읽어 저장·정산·환불이
     * 두 번 반영된다. 상태 컬럼의 조건부 UPDATE 만으로는 답변 저장·정산까지 한 결정 아래 묶지 못한다.
     *
     * <p><b>예약 전이도 이 잠금을 쓴다.</b> 접수 뒤 처리가 길게 지연되면 그 사이 미정산 예약 반환이 요청을
     * 이미 끝냈을 수 있다. 잠그지 않고 예약하면 늦게 재개된 실행이 그 종료를 덮어쓰고 생성을 시작한다.
     *
     * <p>잠금은 트랜잭션이 끝날 때 풀리므로 이 메서드는 반드시 {@code @Transactional} 안에서 부른다.
     * 잠금 대기 시간은 연결 세션의 innodb_lock_wait_timeout(앱이 5초로 못박는다)을 따르며 여기서 따로 걸지 않는다.
     */
    ...
    /**
     * 미정산 예약 반환이 훑을 대상 — 기한이 {@code overdueBefore} 보다 앞선 <b>미종료</b> 요청의 id 목록.
     *
     ...
     *
     * <p>정렬은 기한이 이른 순 — 한 번에 읽어 오는 묶음(Pageable)에 다 담기지 않을 때 오래 묶여 있던
     * 예약부터 돌려준다. 같은 기한이면 id 순으로 고정해 묶음마다 순서가 흔들리지 않게 한다.
