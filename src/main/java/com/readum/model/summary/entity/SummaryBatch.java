...
/**
 * 외부 배치 제공자(현재 OpenAI)에 제출된 배치 요청 하나를 영속화한다.
 * 상태: SUBMITTED(제출 완료, 결과 대기) → COMPLETED(결과 수집 완료) / FAILED(실패 또는 만료).
 * 한 배치에는 여러 감상문 작업(SummaryJob)이 묶인다.
 */
@Getter
@Entity
@Table(name = "summary_batch", indexes = {
        @Index(name = "idx_summary_batch_status", columnList = "status")
})
...
public class SummaryBatch {
    ...
    /** 외부 배치 제공자가 발급한 배치 식별자 (예: batch_abc123). */
    ...
    /** 입력 JSONL 파일 식별자 — 업로드 시 제공자가 발급. */
    @Column(name = "input_file_id", length = 100)
    private String inputFileId;

    /** 결과 JSONL 파일 식별자 — 완료 시 제공자가 발급. */
    @Column(name = "output_file_id", length = 100)
    private String outputFileId;

    /** 오류 JSONL 파일 식별자 — 부분 실패 행이 있을 때 제공자가 발급. */
    ...
    /** 제출 성공 직후 기록. inputFileId 는 업로드 때 받은 식별자. */
    public static SummaryBatch createSubmitted(String batchId, String inputFileId, int jobCount) {
        LocalDateTime now = LocalDateTime.now();
        return new SummaryBatch(null, batchId, Status.SUBMITTED, inputFileId, null, null, jobCount, now, now);
    }
    ...
    /** 배치 실패 또는 제공자 측 오류 — 작업들은 별도로 재큐된다. */
