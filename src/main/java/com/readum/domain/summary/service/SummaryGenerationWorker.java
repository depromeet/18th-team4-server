 ...
 *
 * 호출 전: 전역 차단(quota 소진)이 열려 있으면 작업을 선점하지 않고, RPM/TPM 페이서로 속도를 조인다.
 * 실패 분류: quota 소진 → 전역 차단 후 재시도 / 429 burst → Retry-After 로 재시도 / 4xx → 즉시 종료(FAILED)
 *            / 5xx·기타 → 재시도. (Plan 1 의 "전부 재시도" 를 정교화)
 */
@Slf4j
@Component
