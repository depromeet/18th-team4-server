...
/**
 * 세션에 대한 컨텍스트 요약 작업을, "요약 경계 이후 원문 꼬리 토큰 합 > 임계값"일 때만 멱등하게 적재한다.
...
public class EnqueueContextSummaryJobService {
    ...
    /**
     * 요약 경계 이후 원문 꼬리 토큰 합이 트리거 임계값을 넘으면 요약 작업을 적재한다.
    ...
    public void enqueueIfTailExceedsThreshold(Long sessionId) {
        long boundary = summaryRepository.findBySessionId(sessionId)
                .map(AiChatContextSummary::getSummarizedUntilMessageId)
                .orElse(0L);
        long tailTokens = messageRepository.sumTokenCountAfterId(sessionId, boundary);
        if (tailTokens <= aiChatProperties.context().summarizeTriggerTokenThreshold()) {
            return;
        }
