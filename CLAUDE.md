...
- Lombok: `@Getter`, `@NoArgsConstructor(access = PROTECTED)`, `@AllArgsConstructor(access = PRIVATE)`. 엔티티 생성자 접근 제어를 테스트 편의로 완화하지 않는다 — 운영 코드에서 도메인 팩토리(`create()` 계열)를 우회해 임의 상태 엔티티를 만드는 길을 두지 않는다.
- 정적 팩토리 메서드: 엔티티는 불변식을 강제하는 도메인 팩토리(`create()` 계열 — `create()`/`createInProgress()`/`createChild()` 등)만 노출한다. 특정 id 지정·임의 상태(모든 필드 지정) 객체 생성은 **테스트 전용 책임**이며, 엔티티가 아니라 그 엔티티와 같은 패키지의 `src/test` `{Entity}Fixture` 헬퍼가 같은 패키지에서 접근 가능한 protected 무인자 생성자로 객체를 만들어 `ReflectionTestUtils` 로 필드를 채워 수행한다 (`@com.readum.support.TestOnly` 표식). 운영 엔티티에 `of()` (전체 필드 지정 생성) 를 두지 않는다.
- 픽스처는 **의도가 드러나는 명명 팩토리만** 노출한다 (예: `persistedUser`/`activeToken`/`persistedClosedSession`). 운영 DB 에 존재할 수 있는 유효 상태만, 호출부마다 실제로 달라지는 인자만 받고 나머지는 내부 기본값. 범용 `of()`/`create()` 같은 모든 필드 받는 탈출구는 두지 않는다. `ReflectionTestUtils` 는 픽스처 내부의 private `assemble` 헬퍼에 한정해 쓰고, 외부 API 는 명명 팩토리만 노출한다.
- setter 없이 불변 지향
